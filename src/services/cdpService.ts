        ...
        this.ws.on('message', (msg: WebSocket.Data) => {
            try {
                ...
                if (data.id !== undefined && this.pendingCalls.has(data.id)) {
                    ...
                    if (data.error) reject(new Error(data.error.message || JSON.stringify(data.error))); else resolve(data.result);
                }
                ...
                logger.warn('[CdpService] Failed to parse WebSocket message:', e);
            }
        });
